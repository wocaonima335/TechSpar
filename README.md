...
支持简历模拟面试和专项强化训练两种模式，覆盖 17 个技术领域。新版前端采用更克制的深色工作台视觉、响应式导航与懒加载路由，让训练入口、复盘、画像、知识库和管理后台在桌面与移动端都更聚焦。

### 现代化工作台体验

- **统一设计系统**：新增 `frontend/DESIGN.md`，沉淀配色、排版、卡片、空状态和后台页面规范。
- **低噪声界面**：全局背景、导航、卡片、表单、按钮和内容密度重新梳理，突出训练任务本身。
- **路由懒加载**：核心页面按需加载，减少首屏负担，并提供统一加载态。
- **后台一致性**：用户管理、内容管理、模型设置复用统一后台 Shell，减少页面割裂感。
...
**Admin Runtime Settings** — 管理员可以在后台安全调整运行时 LLM 配置（模型、Base URL、API Key），保存前会执行连接测试；前端只展示必要字段，不暴露 secret、JWT secret 或 embedding 配置。

**Modern Responsive UI** — 基于 React 19、React Router v7、Vite 与 Tailwind CSS v4 的现代化前端。页面级懒加载、统一空/加载/错误态、移动端导航和后台 Shell 让训练与管理体验更一致。
...
本次新增 Qt 开发专项训练，内置 Qt 对象模型、信号槽、事件循环、线程模型、模型视图、绘制、QML/C++ 混合开发、资源与国际化等高频面试主题，并配套独立知识库文档与高频题库。
...
> 管理员也可以在系统设置页调整运行时 LLM 模型、Base URL 和 API Key，并在保存前测试连接。敏感配置会被遮蔽，不会直接暴露到前端。
...
│   ├── runtime_settings.py  # 运行时 LLM 配置持久化
│   ├── runtime_settings_validation.py # 模型连接测试
│   ├── graphs/
...
├── frontend/
│   ├── DESIGN.md            # 前端设计系统与分阶段刷新计划
│   └── src/
│       ├── pages/           # 首页、面试、复盘、画像、知识库等
│       ├── components/      # 通用组件
│       ├── context/         # AuthProvider / useAuth 等上下文
│       └── api/             # API 封装
├── data/
│   ├── topics.json          # 领域配置
│   ├── high_freq/           # 高频题库 Markdown
│   ├── knowledge/           # 各领域知识文档
